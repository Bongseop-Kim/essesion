...
접속 URL은 `DATABASE_URL` env(기본 = compose 값). 데이터 이관 초안은 `scripts/migrate_data.py --help`.
...
- pgvector는 베이스라인 리비전이 `CREATE EXTENSION`으로 활성화(로컬 compose·Cloud SQL 모두 지원). motifs.embedding = vector(1536) 고정.
