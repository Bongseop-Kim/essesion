...
| 11 | 빈 스테이징 DB에 일회성 `bootstrap_admin.py create`로 관리자 생성 + `seed_motifs.py` → `index_motif_embeddings.py --confirm-live` → `backfill_slot_labels.py --confirm-live` → `seed_authoring_examples.py --confirm-live` 실행, motif/example의 `embedded=total`, 슬롯 라벨·부위 백필 `eligible/updated`, admin 표본 확인 (`apps/api/scripts/seed.py`는 local/test 전용) | **사용자** |
...
`eligible=<대상>; updated=<갱신>`을 배포 기록에 남긴다. 백필은 공개 멀티슬롯의
NULL `slot_labels` 또는 `slot_parts`만 각각 조건부 갱신해 재실행이 안전하고
`user_upload`은 제외한다. 완료 뒤 admin Motif 상세에서 `슬롯 → 부위` 표본을
확인한다. GCP project/ADC 또는 `--confirm-live`가 없으면 유료 비전 호출과 DB
갱신을 시작하지 않는다.
