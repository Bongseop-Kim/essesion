...
| 11 | 빈 스테이징 DB에 일회성 `bootstrap_admin.py create`로 관리자 생성 + `seed_motifs.py` → `index_motif_embeddings.py --confirm-live` → `seed_authoring_examples.py --confirm-live` 실행, motif/example의 `embedded=total`과 concrete-color 모티프 admin 표본 확인 (`apps/api/scripts/seed.py`는 local/test 전용) | **사용자** |
...
#                 naver-client-secret openai-api-key
