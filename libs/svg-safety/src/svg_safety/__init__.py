"""공용 SVG 화이트리스트 게이트 (worker-engine.md §2).
...
def _parse(svg: str) -> ET.Element:
    # 1차 방어는 defusedxml(DTD·엔티티 확장을 파서 레벨에서 차단). 문자열 사전검사는
    # 보조 방어선 — 파서 교체·설정 변화로 조용히 재개방되지 않게 이중화한다.
    if "<!DOCTYPE" in svg.upper() or "<!ENTITY" in svg.upper():
        raise SanitizeError("DTD/entity is not allowed")
    try:
        return DefusedET.fromstring(svg)
...
def parse_svg_tree(svg: str) -> ET.Element:
    ...
    normalize 게이트는 geometry 측정·allowlist 검사에 지역 태그명이 필요하므로
...
def scrub_svg(svg: str) -> str:
    ...
    _validate_tree(root)
    # parse_svg_tree가 기본 네임스페이스를 지역명으로 접었으므로 xmlns를 되살린다 —
    # 프로세스 전역 register_namespace 없이 트리 로컬로 처리.
