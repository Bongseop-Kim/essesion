...
|---|---|---|---|---|
| neutral | ✓ (neutral / -muted / -subtle / placeholder / disabled) | ✓ (neutral-solid — 상호작용 변형 없음) | ✓ (neutral-weak +hover/pressed, disabled) | ✓ (neutral / -weak) |
| brand | ✓ | ✓ (+hover/pressed) | ✓ | ✓ |
...
- **solid 선택 채움 = `bg.brand-solid` 재사용** — 체크박스 체크·라디오 도트·Switch on·Chip selected의 "검정 채움"은 별도 selected 토큰 없이 `bg.brand-solid`를 쓴다(#111 모노크롬이라 seed의 neutral-inverted와 동일 효과). 다크모드 도입 시 재평가.
- **포커스된 입력 테두리 = `stroke.brand` 재사용** — TextField 등 포커스 시 2px 검정 테두리는 새 토큰(`stroke.neutral-strong`류) 없이 `stroke.brand`를 쓴다. 다크모드 도입 시 재평가.
- **overlay** — `bg.overlay`는 모달 `<dialog>` `::backdrop` 딤 전용(45% 검정, 정적 값). 면 색·상태 표현에 쓰지 말 것. → [overlay.md](./overlay.md)
- **폼 필드 비활성** — `bg.disabled` + `fg.disabled`(버튼류는 opacity-50 — [state.md](./state.md)), 플레이스홀더는 `fg.placeholder`.
