...
겹쳐 뜨거나 끼어드는 UI 7종의 결정 체계. 생김새가 비슷하므로 **용도로 고른다** — 형태로 고르지 말 것. 값은 [design-token-reference.md](./design-token-reference.md), 모션은 [motion.md](./motion.md).
...
3. 현재 문맥 위에서 **임시 작업·콘텐츠**(폼·필터·상세) → **ResponsiveModal**
   ...
   — 그 내용이 "액션 목록"이면 → **SwipeableMenuSheet**(모바일) / **Menu**(데스크톱)
4. **트리거 요소에 붙는** 소형 명령 목록(데스크톱) → **Menu**
5. 측면 맥락 유지가 중요한 **넓은 보조 작업**(admin 상세·설정) → **SidePanel** — 그 외 데스크톱 보조 작업은 Modal이 기본
6. 특정 섹션에 **상주하는 안내·경고** → **Callout**
7. **페이지 전체 범위**의 공지(페이지당 1개) → **PageBanner**
...
| light-dismiss | Modal · BottomSheet · SwipeableMenuSheet · SidePanel · Menu | 바깥 탭 + Esc(+시트는 스와이프·드래그). 중요한 폼이 든 Modal/BottomSheet는 `showCloseButton` 권장 |
...
- **액션 목록 = SwipeableMenuSheet ↔ Menu** — 자동 스위치 없음, 호출부가 `useBreakpoint()`로 분기.
...
- 딤 = `backdrop:bg-bg-overlay`. 배경 스크롤 잠금 = theme.css의 `body:has(dialog:modal)` 한 줄.
