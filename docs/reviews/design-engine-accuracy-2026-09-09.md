...
- 브라우저 확인은 하지 않았다 — SVG 바이트가 바뀌지 않아 화면에 나타날 차이가 없다.
...
- **Pillow 알파 마스크 겹침 판정**: AABB가 겹친 후보에만 적용하는 2단계다. 마스크 해상도·알파 임계값·안전 여백을
  정하지 않은 채 넣으면 가는 도형에서 오탐한다 — 남은 플랜.
- **정확한 개수 보장(Bridson·Sample Elimination)**: 최소 간격과 지정 개수는 다른 목표이며 기존 바이트가 바뀐다.
  7단계 실측 전에는 판단하지 않는다 — 남은 플랜.
- **저작 컴파일러의 salt**: 위 재현성 판정 참조. 실제 포개짐 사례가 저작 경로에서 확인되면 그때
  `COMPILER_REVISION`과 함께 다룬다.
- **곡선 bbox 정밀화**: 과대추정과 보이는 크기 차이를 fixture로 측정하지 않았다 — 남은 플랜.
