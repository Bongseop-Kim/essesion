...
`WORKER_FINALIZE_INLINE`은 `.env` 열람 금지 규칙상 직접 확인하지 않고 S9-1의
...
| S9-2 실사화 취소 | 미검증 | `WORKER_FINALIZE_INLINE=true`에서 작업이 요청 내에 완료돼 "취소하고 횟수 되돌리기"가 노출되지 않는다. 이 플래그로는 도달 불가한 경로다. |
