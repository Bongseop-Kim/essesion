...
**R10. api 경계 입력 선검증**
- 당시 상태: api 요청의 후보 개수 필드에 ge/le가 없어 큰 값이 워커까지 전달되고 422→불필요한 환불 사이클이 발생할 수 있었다.
- 적용 결과: API 경계에서 범위를 검증하고 boundary test와 OpenAPI 생성물로 드리프트를 막았다. **후속**: 디자인 재설계로 후보 개념 자체가 폐기돼(생성 1건 = 디자인 1개) 이 필드는 사라졌고, 남은 교훈은 "워커에 닿기 전 api 경계에서 범위를 막는다"다.
