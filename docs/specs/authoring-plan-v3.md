...
`/few-shot-examples/new`에서 admin은 다음 흐름으로 `authored` 시범을 만든다.
...
관리자 화면은 `/few-shot-candidates`(후보 검토)와 `/few-shot-examples`(선별된 few-shot)로 분리되어
있고, 직접 저작은 `/few-shot-examples/new` 전용 페이지다.
