...
- 업로드: api가 GCS **서명 업로드 URL** 발급(엔드포인트가 object_key 결정) → 클라 업로드 → 등록 엔드포인트. 수선 사진은 비회원도 발급할 수 있으며 JPG/PNG/WebP·10MiB 상한, 15분 PUT URL, 24시간 claim token을 적용한다. 완료 등록에서 GCS metadata(size/content-type)를 검증하고, 로그인 장바구니 동기화 시 사용자 소유로 전환한다.
