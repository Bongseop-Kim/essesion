...
- 전체 교체(replace): 유저 advisory lock → DELETE 전부 → 배열 재삽입. 빈 배열 = 비우기. quantity 양의 정수 검증. product면 product_id 필수·reform_data 금지, reform이면 반대(제약).
...
- 생성: quantity ≥ 100, options는 object(10KB 제한), contact_method ∈ {email, phone}, contact_name/value 필수, 배송지 본인 소유. reference_images `[{url, file_id?}]` → images 등록(entity_type='quote_request'). `QUO-` 채번. status='요청'. 접수 알림톡(수신 조건 4종): `[ESSE SION] 견적 요청이 접수되었습니다.\n담당자가 순차적으로 연락드리겠습니다.`
...
- 업로드: api가 GCS **서명 업로드 URL** 발급(엔드포인트가 object_key 결정) → 클라 업로드 → 등록 엔드포인트.
...
- 만료: 업로드 시 NULL, 견적 확정·종료 +90일(§7). 정리 배치: `deleted_at IS NULL AND (expires_at < now() OR deletion_claimed_at IS NOT NULL)` 배치 100건 — ①claim(deletion_claimed_at=now) ②GCS 삭제 ③성공분 deleted_at=now (2단계 멱등 삭제).
