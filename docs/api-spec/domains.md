...
- **네이버앱 자동로그인**(개발가이드 §5.1): `GET /auth/naver/login?auth_type=autologin` — authorize URL에 `auth_type=autologin`을 붙인다(네이버 전용, 타 provider는 무시). 시도 조건은 store가 판단: 네이버앱 UA(`NAVER(inapp; search;`) + 과거 네이버 로그인 이력(localStorage) + 브라우저 세션당 1회(sessionStorage). provider가 오류(동의 취소·자동로그인 실패)로 콜백하면 `?error=access_denied`(아래 콜백 실패 계약) — store는 자동로그인 시도였으면 조용히 홈으로, 수동 취소였으면 안내 후 로그인 페이지로.
- **소셜 콜백 실패 계약**: OAuth 콜백은 브라우저 최상위 내비게이션의 종착지라 **어떤 실패도 JSON을 반환하지 않는다**(반환하면 API 콜백 URL에 응답 본문이 그대로 화면에 뜬다). 전부 `{frontend_origin}/auth/callback?error=<code>`로 303. code는 `access_denied`(provider 단계 — 동의 취소·자동로그인 실패·토큰 교환 실패) / `account_unavailable`(계정 단계 — 비활성·비고객 계정 거부) / `server_error`(그 외 전부; 상세는 서버 로그에만). store는 code로 문구를 고르고, 자동로그인 시도였으면 code와 무관하게 조용히 홈으로.
