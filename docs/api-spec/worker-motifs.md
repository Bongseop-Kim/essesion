...
원본: `app/motifs/`, `app/adapters/`. 모티프 검색 래더·content-hash·프롬프트는 기능 명세의 일부 — 원문 보존. DB 스키마는 새 모노레포의 `motifs` 테이블(Vertex `vector(3072)`, legacy `vector(1536)` 호환 컬럼 — db/MAPPING.md).
...
- 기존 공개 NULL 행은 `apps/worker/scripts/backfill_motif_embeddings.py --confirm-live`가 UPDATE한다. GCP project/ADC·확인 플래그가 없으면 실행을 거부하고 `user_upload`은 제외한다. 기존 1536차원 값은 무중단 롤백용으로 보존하지만 검색에는 사용하지 않는다.
...
- Git `gallery-v1`은 25개 golden intent에서 추출한 reviewable Plan v3 예시다. golden SHA와 manifest 재생성 결과를 테스트하며, `authoring_examples`에는 revision별 불변 projection과 Vertex embedding만 둔다. query top-25를 motif 수/배치 제약으로 거른 상위 8개에서 family 다양성을 우선해 최대 3개를 prompt에 넣는다. retrieval 장애·빈 revision은 예시 없이 계속하는 fail-soft다.
- 기본 `legacy`, request-id 기반 `shadow`/`canary`, 전면 `v3` 모드를 배포 env로 선택한다. shadow v3는 30초 한도에서 결과를 폐기하고 legacy만 응답한다. canary/v3는 실패를 legacy로 숨기지 않는다. contract/compiler/prompt/example revision, retrieval 상태·선택 ID/유사도, fingerprint는 generation diagnostics와 intent log에 남긴다.
- live 비교는 `eval_authoring.py --confirm-live --pipeline legacy --pipeline v3`의 label 30-case corpus로 schema/compiler 성공률, 구조 다양성, retrieval expected-family recall, 시도 수와 latency를 측정한다. prompt/provider 원문은 출력·저장하지 않고 CI는 유료 호출을 실행하지 않는다. 정본·동기화·승격 절차는 `docs/specs/authoring-plan-v3.md`다.
