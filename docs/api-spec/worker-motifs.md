...
- Gemini는 전체 엔진 intent를 직접 만들지 않는다. structured output은 `{"plans":[...]}` 2~4개이며 각 plan은 최대 2개의 의미 모티프(`subject`, `scope`, optional style/description), HEX 색 2~5개, `arrangement`, `density`, `scale`, `direction`, `stripes`만 가진다.
...
- exact private motif의 실제 ID는 Gemini에 전달하지 않고 개수만 알린다. compiler가 최대 2개를 우선 삽입한다. resolver가 선택한 DB motif가 다중색이면 실제 `color_slots` 전체를 palette slot에 결정적으로 결합한 뒤 후보를 검증한다.
