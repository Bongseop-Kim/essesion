...
- 두 편집은 `asyncio.gather`로 **병렬** — p95는 편집 1회분(실측 ~25s, quality=low — 단가와 한 몸, money.md §6)에 가깝다.
...
- 지명색 어휘(`adapters/named_colors.py`)는 navy·burgundy·ivory·gold·white를 표로 고정해 두지만, **문장이 색 이름 바로 옆에 hex를 적었으면 그 hex가 이긴다**("금색 #FFD700" → 표의 `#D4AF37`이 아니라 `#FFD700`). 이름만 있으면 종전대로 표 값으로 접지한다. 2026-09-09 정확도 평가에서 표가 사용자가 적은 값을 덮어쓰는 것을 실측해 고쳤다.
- 순수 모티프 요청처럼 patch로 적용할 축이 없으면 worker는 **HTTP 200 `{status:"scope_rejected",reason?,motif_intent?}`**를 돌려준다. `reason`은 `DesignPatchV1.out_of_scope_reason`을 옮긴 것이거나 worker가 붙인 `no_change`(patch를 적용했는데 intent가 현재와 같음 — 이미 최소 밀도인데 더 성기게 등; 모델 note는 버린다)다: `motif_change`(모티프 교체·추가·삭제), `motif_recolor`(모티프 또는 그 일부 재채색 — 색은 모티프 identity라 store가 AI 생성으로 새 그림을 만들라고 안내한다), `motif_position`(patch로 표현할 수 없는 위치 — 줄무늬가 없는 디자인의 "줄 사이", 임의 좌표 등; 줄무늬가 있으면 `arrangement=on_stripes|between_stripes`가 지원 축이다), `target_missing`(`motifs` 목록에 없거나 어느 모티프인지 모호한 대상 지정). 이 중 **`motif_change`(또는 reason 없음)만** `motif_intent` 피커 sidecar를 동반할 수 있다 — 나머지 네 이유는 이 계약으로 표현 못 하는 축이라 피커가 도움이 안 되므로 store가 이유별 한글 안내 문구로 보여준다. 디자인을 만들지 않았으므로 api는 `work_id` 멱등 환불로 과금을 되돌리고, 요청 턴을 지우고 `context_version`을 원복해 공개 응답 `200 {rejected:"motif",motif_intent?}`를 낸다 — 이력에 스텝이 남지 않는다. 모티프 요청과 바탕·줄무늬 같은 지원 축이 섞이면 지원 축만 적용하고(편집 1회 과금) 일반 성공 응답에 같은 sidecar를 싣는다. sidecar가 없는 거절은 store가 빨강 알림 1건으로 알린다.
- `warnings`는 `[{code, message}]`다. 엔진·리졸버의 영문 진단 문자열은 로그·`diagnostics`의 정본으로 남기고, `worker.warnings.WARNING_MESSAGES`에 한글 문구가 있는 코드만 응답에 담는다(코드별 1건). 매핑에 없는 경고는 고객에게 노출하지 않는다. 문구를 두는 기준은 **요청과 다른 결과가 나왔고 화면만 보고는 알 수 없는 것**(색역·모티프 드랍·색 미배치·근사 매칭)이며, 캔버스에서 보이는 자동 맞춤(크기 클램프·간격 스냅·줄 너비 축소·산개 개수 미달)과 고객이 손쓸 수 없는 실패(프리뷰 업로드)는 로그·admin `warning_groups`에만 남긴다.
