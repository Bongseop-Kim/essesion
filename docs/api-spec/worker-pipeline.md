...
- `POST /generate` — 무세션 worker 계약. API가 부여한 필수 `run_id`와 함께 `prompt` 저작 또는 `intent`+새 seed 변형을 받으며 별도 mode 필드는 두지 않는다. ordered `reference_images[{image_id,url,content_type,size_bytes,purpose}]`(최대 5), exact `motif_ids`(최대 2), `palette`, `pattern_constraints`, `candidate_count`(1..8)를 받는다. prompt 요청에는 API가 검증한 내부 `motif_provenance{user_id,session_id}`도 전달해 Recraft content-hash 행의 최초 유입만 감사 태깅한다. variation direct intent에는 새 유입 provenance를 보내지 않으며 catalog hit는 기존 값을 덮지 않는다. 사진 purpose·색·패턴은 프롬프트 문자열로 합치지 않고 worker 엔진까지 구조적으로 전달한다. 명확한 개별 도형이 catalog miss면 자동 생성할 수 있지만 한 요청의 authored design과 적합성 retry 전체가 실제 Recraft 호출 기본 2회 상한을 공유하고 초과 best-effort layer는 경고와 함께 drop한다. **응답은 원본과 달리 풍부하게**: 내부 서비스이므로 후보별 `{id, design_index, layout_id, source_fidelity, colorway_id, seed, svg, png_object_key}` + `{generation_log_id, request_id, registry_version, engine_version, warnings}` 반환 — api는 `generation_log_id == run_id`를 검증하고 공개 응답·assistant turn에는 `run_id`만 노출한다.
- 사용자 수정 가능한 422는 `{detail:{code,stage,message}}` 고정 계약이다. code는 `reference_invalid|constraint_conflict|authoring_invalid|semantic_mismatch|intent_invalid|candidate_invalid`, stage는 각각 `reference|constraints|authoring|authoring|intent|candidate`다. API는 exact+motif 사진이 2슬롯을 넘으면 worker 호출·과금 전에 `motif_input_conflict`를 반환한다. 원문 오류는 노출하지 않고 code/stage별 한국어 메시지로 투영하며 과금 뒤 generate worker 실패는 기존과 같이 환불한다.
...
- seamless_generation_logs INSERT는 워커가 직접(원 동작 — system of record, SVG 재-export 근거). `diagnostics` JSONB에는 mode, 모델·prompt revision, 저작 시도/plan/검증/해석/후보 수, 단계별 시간, 모티프별 exact/embedding/catalog/Recraft/drop 결과와 실패 code/stage/provider/operation/reason/status를 저장한다. worker JSON 로그에도 같은 안전한 provider 식별 필드만 넣으며 provider 응답·인증 header·프롬프트 원문·내부 예외는 넣지 않는다.
