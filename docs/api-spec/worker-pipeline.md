...
- seamless_generation_logs INSERT는 워커가 직접(원 동작 — system of record, SVG 재-export 근거). `diagnostics` JSONB에는 mode, 모델, 저작 시도/plan/검증/해석/후보 수, 실패 code/stage와 구조화 설정 사용 여부만 저장하며 provider 응답·프롬프트 원문·내부 예외는 넣지 않는다.
