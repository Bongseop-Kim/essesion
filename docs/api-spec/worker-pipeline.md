...
- 순수 모티프 요청처럼 patch로 적용할 축이 없으면 worker는 **HTTP 200 `{status:"scope_rejected",reason?,motif_intent?}`**를 돌려준다. `reason`은 `DesignPatchV1.out_of_scope_reason`을 그대로 옮긴 것으로 다섯 값 중 하나다: `motif_change`(모티프 교체·추가·삭제), `motif_recolor`(모티프 또는 그 일부 재채색), `motif_position`(줄무늬 대비 상대 위치 이동), `per_motif_placement`(여러 모티프 중 하나만 회전·간격·밀도 변경), `target_missing`(`motifs` 목록에 없거나 어느 모티프인지 모호한 대상 지정). 이 중 **`motif_change`(또는 reason 없음)만** `motif_intent` 피커 sidecar를 동반할 수 있다 — 나머지 네 이유는 이 계약으로 표현 못 하는 축이라 피커가 도움이 안 되므로 store가 이유별 한글 안내 문구로 보여준다. 디자인을 만들지 않았으므로 api는 `work_id` 멱등 환불로 과금을 되돌리고, 요청 턴을 지우고 `context_version`을 원복해 공개 응답 `200 {rejected:"motif",motif_intent?}`를 낸다 — 이력에 스텝이 남지 않는다. 모티프 요청과 바탕·줄무늬 같은 지원 축이 섞이면 지원 축만 적용하고(편집 1회 과금) 일반 성공 응답에 같은 sidecar를 싣는다. sidecar가 없는 거절은 store가 빨강 알림 1건으로 알린다.
