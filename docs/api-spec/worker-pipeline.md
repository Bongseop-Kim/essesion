...
- `POST /finalize` — `/export`와 같은 급의 **stateless 동기 엔드포인트**. 입력은 `GenerationJob.params`와 동일 형태(`{intent, colorway_id?, production_method?, dpi?, weave?, material_map?, texture_strength?, relief_strength?}`, strict — 계약 밖 필드 422), 값 검증은 render_fabric이 최종 권위. 처리: render_fabric → GCS `fabric/{sha256[:16]}.png` create-only 업로드(`if_generation_match=0`, 같은 내용의 기존 객체 412는 멱등 성공) → `{object_key}` 반환. DB claim·FOR UPDATE·lease 없음 — 잡 상태는 api가 소유한다.
