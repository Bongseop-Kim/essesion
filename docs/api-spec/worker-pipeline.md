...
- 지명색 어휘(`adapters/named_colors.py`)는 좁은 표에 hex를 고정해 두지만, **문장이 색 이름 바로 옆에 hex를 적었으면 그 hex가 이긴다**("금색 #FFD700" → 표의 `#D4AF37`이 아니라 `#FFD700`). 이름만 있으면 종전대로 표 값으로 접지한다. 2026-09-09 정확도 평가에서 표가 사용자가 적은 값을 덮어쓰는 것을 실측해 고쳤다.
