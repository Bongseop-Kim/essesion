...
- [x] 생성 실패 진단·오류 계약 — *reference/constraints/authoring/intent/candidate 단계별 고정 422 code와 환불을 API·store까지 보존하고, `seamless_generation_logs.diagnostics` JSONB/Alembic·admin safe projection을 추가했다. 프롬프트·provider 응답·raw 내부 오류는 진단에 저장하거나 사용자에게 노출하지 않는다.*
