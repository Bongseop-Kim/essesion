...
- [x] Gemini Plan v3 + RAG few-shot 저작 파이프라인 — *25개 이상 골든을 SHA로 묶은 Git `gallery-v1`, strict typed `DesignPlansV3`와 결정적 compiler, immutable `authoring_examples`/Vertex 3072d pgvector projection, 호환성·family 다양성 기반 최대 3개 예시 선택, typed Vertex `response_schema`, 구조 중복 retry, revision/예시/구조 fingerprint 진단을 구현했다. 기본은 legacy이며 request-id 기반 shadow/canary/v3를 배포 env로만 승격한다. 개발자용 build/check·sync·30-case legacy/v3 평가 도구와 실제 PostgreSQL 회귀 테스트를 포함하고, 관리자 페이지 및 런타임 prompt/settings 편집은 범위에서 제외했다 (`docs/specs/authoring-plan-v3.md`).*
