...
- [ ] Alembic 스테이징 적용 — *첫 배포의 migrate Cloud Run job 성공과 현재 head(`c7a8d2f1b604`) 확인. 빈 DB에는 베이스라인 `f8c3b2a19d47` 뒤 OpenAI 전환·모티프 정리·design session 예산 컬럼 보정 리비전이 순서대로 적용되며, 적용 뒤 OpenAI 임베딩을 다시 만든다.*
...
- [ ] 빈 스테이징 DB를 현재 Alembic head `c7a8d2f1b604`까지 적용 → 관리자·고정 색상 motif 초기 입력 → `backfill_motif_tags.py --confirm-live` → `index_motif_embeddings.py --confirm-live` → `seed_authoring_examples.py --confirm-live` 순서로 실행, motif/example `embedded=total` 및 admin 표본의 SVG 색상·자동 태그·편집 결과 검증
