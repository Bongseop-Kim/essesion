...
- **제외 (이연)**: 견적 내역 조회 페이지(`GET /quotes` — C9 마이페이지), success 페이지의 custom 전용 문구 분기(기본 "결제가 완료되었습니다"로 충분), 알림톡은 서버 BackgroundTasks가 처리하므로 프론트 무관.
...
/custom-order (공개)
  ...
  │    (원단 섹션은 fabricProvided||reorder면 숨김)
...
| 견적 접수 후 마이페이지 견적 목록으로 이동 | snackbar + **홈 이동** (견적 목록은 C9) | 청크 경계. `GET /quotes` 준비돼 있어 C9에서 붙이기만 |
| 결제 페이지가 zustand 없이 location.state (`CustomOrderPaymentState`) | 동일 — router state draft, 새로고침 시 `/custom-order` 복귀 | C3 D4와 같은 패턴. 위저드 폼도 메모리만(원본 동일, persist 안 함) |
| 연락처 프로필 자동 채움(`quote-contact-defaults`) | 로그인 세션의 name/phone/email로 동일 재현 | 보존 |

보존: 6섹션 스크롤 위저드 + 검증 실패 시 해당 섹션 스크롤, 옵션 의존성 리셋(fabricProvided→reorder/원단 리셋, 수동 타이→딤플 해제+안내), 100개 임계 안내, 수량 프리셋 `[4,8,12,20,50,100]`·최소 4개, 넥타이 폭 6~12cm 0.5 단위 clamp, 예상 제작 기간(직접 제공 7~14일/재주문 21~28일/기본 28~42일), 견적 모드 담당자 정보 필수 + 확인 다이얼로그, 선주문 생성 → Toss → confirm 순서.
...
- 제출 시: 스텝 배열 순회 → `visible=false`(숨긴 원단 섹션) 스킵 → 첫 실패 스텝에서 중단 → `snackbar(에러 문구)` + `scrollIntoView({behavior:"smooth"})` + 첫 오류 필드 focus. 통과 후 견적모드/즉시주문 분기.
- 인라인 에러: 제출 시도 전에는 표시하지 않음(RHF `submitCount`) — 작성 중 빨간 화면 방지. 시도 후엔 필드별 `errorMessage`(Field/TextField 배선).
- 실검증 규칙(원본 보존): quantity ≥ 4 / 원단: fabric_provided·reorder 아니면 design+fabric 필수 / tie_width 6~12(입력 시 clamp+0.5 정규화라 사실상 통과) / 견적 모드: contact_name·contact_value 필수 / 봉제·마감·첨부는 검증 없음.
...
| sidebar | `SummaryCard`(원단/봉제/수량 요약 Row + 견적 Total) + estimate-summary(단가·예상 제작 기간 caption) + 배송비 유의 `Callout`(neutral) |
...
| D7 | 견적 접수 완료 = snackbar + 홈. 견적 목록은 C9(`GET /quotes` 대기) | 권장 |
...
2. **features/custom-order model**: options 상수 → zod 스키마·스텝 정의 → `to-request.ts`(+단위테스트: interlining 정규화·fabric 조건·기록용 키) → `use-custom-quote.ts`.
3. **섹션 UI 6종** + wizard-section + quote-contact-fields + estimate-summary.
4. **api/upload.ts** + AttachmentDisplayField 연결(requireAuth 가드).
5. **pages/custom-order/index.tsx**: 조립 + 의존성 리셋 + 검증-스크롤(§5-C) + 견적/즉시 분기(§5-B).
...
8. **검증**: `pnpm lint` → `pnpm turbo typecheck test` → 실렌더 왕복 — 로컬 API(Toss DryRun)로 ① 사양 조합 변경→견적 갱신(디바운스·이전값 유지) ② 4개 미만/원단 미선택 검증 스크롤 ③ 수동 타이 딤플 해제 ④ 즉시주문: 결제창→success confirm ⑤ 100개 견적: 담당자+배송지→AlertDialog→접수 snackbar ⑥ 비로그인 견적 표시+제출 가드 ⑦ custom-payment 직접 진입/새로고침 복귀 ⑧ fail 복귀 분기. 반응형(모바일 하단 actionBar) 확인.
