...
- The table container has `min-width: 0`. Remaining horizontal overflow is provided only by `ScrollFog` as documented in [`packages/shared/docs/foundation/scroll.md`](../packages/shared/docs/foundation/scroll.md). Its region is focusable and has a purpose-specific accessible name.
