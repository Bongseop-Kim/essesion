...
2. 준비 상태 확인: 배포 revision이 정상 기동했고 `curl -fsS 'https://api.essesion.shop/readyz'`가 200이며 `database=ready`, `toss/solapi/finalize_tasks=real`, `worker/oauth_google/oauth_kakao/auth_secrets/edge_proxy=ready`, `batch_auth=oidc`인지 확인한다. GCS 버킷·Cloud Tasks 필수 값이 빠지면 revision 자체가 기동하지 않으므로 Cloud Run 로그와 주입값을 먼저 확인한다. `curl -sS -o /dev/null -w '%{http_code}' "$(tofu -chdir=infra output -raw api_url)/readyz"`는 exact edge header 없이 403이어야 한다.
3. `roles/run.invoker`가 있는 점검 계정으로 `worker_generate_url`과 `worker_finalize_url`의 비공개 `/readyz`를 identity token과 함께 호출해 둘 다 `database=ready`인지 확인한다. 명령은 `infra/README.md`의 readiness 절차를 사용한다.
...
4. 공개 `https://api.essesion.shop/readyz`와 admin 대시보드에서 `finalize_tasks`(Cloud Tasks 전달)가 `real`, `batch_auth`가 `oidc`, `edge_proxy`가 `ready`인지 확인한다. GCS 버킷·Cloud Tasks 누락은 capability가 아니라 revision 기동 실패로 드러나며, 다른 capability가 `unavailable`이면 관련 mutation을 진행하지 않는다.
