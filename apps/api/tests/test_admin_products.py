...
async def test_legacy_product_image_urls_survive_omitted_patch_and_allow_explicit_detail_clear(
    client, db_session, settings
):
    admin = await make_admin(db_session)
    headers = auth_headers(admin, settings)
    product = await make_product(db_session, name="레거시 이미지 상품")
    product.image = "https://legacy.example/primary.webp"
    product.detail_images = ["https://legacy.example/detail-one.webp"]
    await db_session.commit()
