  ...
  return (
    <VStack gap="x6" alignItems="stretch">
      ...
      <AdminCard title="클레임 정보">
        <DetailList
          items={[
            ...
            { label: "사유", value: data.reason },
            { label: "상세 사유", value: data.description ?? "-" },
            { label: "수량", value: `${data.quantity}개` },
            { label: "접수 시각", value: formatDateTime(data.created_at) },
      ...
      <AdminCard title="고객·주문 항목">
        <DetailList
          items={[
            ...
            { label: "주문 금액", value: formatMoney(data.order.order_amount) },
            { label: "항목 유형", value: data.item.item_type },
            { label: "항목 ID", value: data.item.item_id },
            ...
            items={[
              {
                ...
                value: shippingAddress
                  ? `${shippingAddress.postal_code} ${shippingAddress.address} ${shippingAddress.address_detail ?? ""}`
                  : "-",
              },
              {
                label: "받는 분",
              ...
              },
              {
                ...
                value:
                  [
                    data.shipping.order_courier_company,
                    data.shipping.order_tracking_number,
                  ]
                    .filter(Boolean)
                    .join(" · ") || "-",
              },
              {
                ...
                value:
                  [
                    data.shipping.company_courier_company,
                    data.shipping.company_tracking_number,
                  ]
                    .filter(Boolean)
                    .join(" · ") || "-",
              },
              ...
              <HStack gap="x2" wrap>
                {(data.tracking_actions ?? []).map((action) => (
                  <ActionButton
                    ...
                    size="small"
                    variant="neutralOutline"
                    disabled={!action.enabled || trackingMutation.isPending}
                ...
                {(notification.delivery_status === "failed" ||
                  ...
                  <ActionButton
                    ...
                    loading={
                      ...
                      notificationMutation.variables?.path.notification_id ===
                        notification.id
                    }
      ...
      <AdminCard title="처리 타임라인">
        {timeline.length === 0 ? (
          <Text color="fg.neutral-muted">기록된 처리 이력이 없습니다.</Text>
        ) : (
          <VStack as="ol" gap="x3" alignItems="stretch">
            {timeline.map((event, index) => (
              ...
              >
                ...
                <Text textStyle="caption" color="fg.neutral-muted">
                  {formatDateTime(event.created_at)} · 처리자{" "}
                  {formatIdentifier(event.actor_id)}
                </Text>
              </VStack>
            ))}
          </VStack>
      ...
      <AlertDialog
        ...
        description={`클레임 ${data.claim_number}의 상태와 운영 이력에 반영됩니다. 입력한 사유: ${memo.trim() || "없음"}`}
        primaryActionProps={{
          children: "실행",
