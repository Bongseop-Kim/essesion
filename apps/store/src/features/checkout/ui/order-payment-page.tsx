import type { ShippingAddressOut, UserCouponOut } from "@essesion/api-client";
import {
  listAddressesOptions,
  listMyCouponsOptions,
} from "@essesion/api-client/query";
import {
  ActionButton,
  ContentPlaceholder,
  Divider,
  Skeleton,
  snackbar,
  Text,
  VStack,
} from "@essesion/shared";
import { useQuery } from "@tanstack/react-query";
import { type ComponentProps, useEffect, useState } from "react";

import {
  CouponSelectModal,
  couponDiscount,
  couponLabel,
} from "@/features/coupon";
import { AddressSelectModal, ShippingAddressCard } from "@/features/shipping";
import { krw } from "@/pages/shop/constants";
import { useSession } from "@/shared/store/session";
import { SummaryCard } from "@/shared/ui/summary-card";

import {
  CHECKOUT_PENDING_KEY,
  useCheckoutPayment,
} from "../model/use-checkout-payment";
import { CheckoutShell } from "./checkout-shell";

/**
 * 배송지·쿠폰·Toss 결제를 조립하는 주문서 결제 페이지 공통부.
 * custom/sample 결제가 공유하며, draft 파싱·폴백 라우팅과 주문 생성 API는
 * 각 페이지가 소유한다. snapshotBase 키 순서는 pending 서명(JSON.stringify)에
 * 반영되므로 임의로 바꾸지 않는다.
 */
export function OrderPaymentPage({
  breadcrumbs,
  title,
  orderName,
  original,
  paymentRowLabel,
  summaryDescription,
  specTitle,
  specRows,
  snapshotBase,
  createOrder,
}: {
  breadcrumbs: ComponentProps<typeof CheckoutShell>["breadcrumbs"];
  title: string;
  orderName: string;
  original: number;
  paymentRowLabel: string;
  summaryDescription: string;
  specTitle: string;
  specRows: readonly { label: string; value: string }[];
  snapshotBase: Record<string, unknown>;
  createOrder: (context: {
    addressId: string;
    couponId: string | null;
  }) => Promise<{ paymentGroupId: string; totalAmount: number }>;
}) {
  const user = useSession((state) => state.user);
  const addressesQuery = useQuery(listAddressesOptions());
  const couponsQuery = useQuery(
    listMyCouponsOptions({ query: { active_only: true } }),
  );
  const [address, setAddress] = useState<ShippingAddressOut | null>(null);
  const [addressModalOpen, setAddressModalOpen] = useState(false);
  const [couponModalOpen, setCouponModalOpen] = useState(false);
  const [coupon, setCoupon] = useState<UserCouponOut | null>(null);

  useEffect(() => {
    if (!address && addressesQuery.data?.[0])
      setAddress(addressesQuery.data[0]);
  }, [address, addressesQuery.data]);

  const discount = couponDiscount(coupon?.coupon, original);
  const total = original - discount;
  const payment = useCheckoutPayment({
    storageKey: CHECKOUT_PENDING_KEY,
    snapshot: {
      ...snapshotBase,
      shippingAddressId: address?.id ?? null,
      userCouponId: coupon?.id ?? null,
    },
    orderName,
    expectedAmount: total,
    createOrder: async () => {
      if (!address) throw new Error("shipping address is required");
      return createOrder({
        addressId: address.id,
        couponId: coupon?.id ?? null,
      });
    },
  });

  const canPay = !!user && !!address && total > 0;

  return (
    <CheckoutShell
      breadcrumbs={breadcrumbs}
      amount={total}
      customerKey={user?.id ?? null}
      summary={
        <SummaryCard.Root>
          <SummaryCard.Section
            title="결제 금액"
            description={summaryDescription}
          />
          <Divider />
          <SummaryCard.Row
            label={paymentRowLabel}
            value={`${krw.format(original)}원`}
          />
          <SummaryCard.Row
            label="쿠폰 할인"
            value={`-${krw.format(discount)}원`}
            tone={discount > 0 ? "informative" : "neutral"}
          />
          <SummaryCard.Row label="배송비" value="0원" />
          <SummaryCard.Total
            label="결제 예정 금액"
            value={`${krw.format(total)}원`}
          />
        </SummaryCard.Root>
      }
      payDisabled={!canPay}
      payLoading={payment.isPending}
      helperText={!address ? "배송지를 먼저 등록해 주세요." : undefined}
      onPay={(widget) => void payment.pay(widget)}
    >
      <VStack gap="x6" alignItems="stretch">
        <VStack gap="x2">
          <Text as="h1" textStyle="title1">
            {title}
          </Text>
          <Text textStyle="body" color="fg.neutral-muted">
            배송지, 쿠폰, 결제 수단을 확인해 주세요.
          </Text>
        </VStack>

        {addressesQuery.isPending ? (
          <Skeleton width="100%" height={120} />
        ) : addressesQuery.isError ? (
          <ContentPlaceholder
            title="배송지를 불러오지 못했습니다"
            description="잠시 후 다시 시도해 주세요."
            action={
              <ActionButton
                type="button"
                variant="neutralOutline"
                onClick={() => void addressesQuery.refetch()}
              >
                다시 시도
              </ActionButton>
            }
          />
        ) : (
          <ShippingAddressCard
            address={address}
            onChange={() => setAddressModalOpen(true)}
          />
        )}

        <VStack gap="x3" alignItems="stretch">
          <Text as="h2" textStyle="title3">
            {specTitle}
          </Text>
          <SummaryCard.Root>
            {specRows.map((row) => (
              <SummaryCard.Row
                key={row.label}
                label={row.label}
                value={row.value}
              />
            ))}
          </SummaryCard.Root>
        </VStack>

        <VStack gap="x3" alignItems="stretch">
          <Text as="h2" textStyle="title3">
            쿠폰
          </Text>
          <Text textStyle="bodySm" color="fg.neutral-muted">
            {coupon
              ? `${couponLabel(coupon.coupon)} 적용`
              : "적용된 쿠폰이 없습니다."}
          </Text>
          <ActionButton
            type="button"
            variant="neutralOutline"
            onClick={() => setCouponModalOpen(true)}
          >
            쿠폰 선택
          </ActionButton>
        </VStack>
      </VStack>

      <AddressSelectModal
        open={addressModalOpen}
        selected={address}
        onOpenChange={setAddressModalOpen}
        onSelect={setAddress}
      />
      <CouponSelectModal
        coupons={couponsQuery.data ?? []}
        selected={coupon}
        loading={couponsQuery.isFetching}
        error={couponsQuery.isError}
        open={couponModalOpen}
        onOpenChange={setCouponModalOpen}
        onApply={async (next) => {
          setCoupon(next);
          setCouponModalOpen(false);
          snackbar(next ? "쿠폰을 적용했습니다." : "쿠폰 적용을 해제했습니다.");
        }}
      />
    </CheckoutShell>
  );
}
