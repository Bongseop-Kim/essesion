...
def _normalize(raw_svg: str, **kwargs):
    return normalize_motif_svg(raw_svg, id_prefix="fixture", **kwargs)
...
def test_concrete_color_is_part_of_motif_identity():
    ...
    assert red.id.startswith("fixture-")
...
def test_determinism_repeated_normalization_is_identical():
    ...
    a = _normalize(svg, render_check=False)
    b = _normalize(svg, render_check=False)
...
def test_root_fill_is_inherited_instead_of_dropped_with_the_root_element():
    ...
    assert _normalize(colored.preview_svg, render_check=False).id == colored.id
...
def test_rejects_raster_image_via_allowlist():
    with pytest.raises(SanitizeError):
        _normalize(_svg('<image href="data:x" width="10" height="10"/>'), render_check=False)
...
def test_rejects_missing_viewbox_and_size():
    ...
    with pytest.raises(ValueError, match="viewBox or positive"):
        _normalize(no_frame, render_check=False)
